...
namespace QuickRoute.UI.Classes
{
  public static class Util
  {
    ...
    public static string PathShortener(string path, int maxLength)
    {
      string ellipsisChars = "...";
      char dirSeparatorChar = Path.DirectorySeparatorChar;
      string directorySeparator = dirSeparatorChar.ToString();
      ...
      for (int i = 0; i < pathParts.Length; i++)
      {
        ...
        else
        {
            ...
            string partToAdd = directorySeparator + pathParts[index];
            if ((firstPart.Length + lastPart.Length + partToAdd.Length + ellipsisLength) > maxLength)
            {
                break;
            }
